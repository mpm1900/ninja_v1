...
function BattleContextController() {
  ...
  const actors = game.actors.filter(a => a.player_ID === context.source_player_ID)
  const active_actors = actors.filter(a => !!a.position_ID)
  const actionable_actors = active_actors.filter(a => !a.stunned && !hasAction(a.ID, game.actions))

  useEffect(() => {
    const first = actionable_actors[0]
    if (!first) return

    setContextSource(first.ID, first.actions[0].ID)
